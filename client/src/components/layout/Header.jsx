...
// BASE_URL untuk gambar yang disajikan oleh backend
...
export default function Header({ onSearch, searchQuery, onResetSearch, onCreateClick }) {
  ...
  let avatarSrc = '/img/default-avatar.png'; // Default avatar lokal

  if (user && user.avatar_url) {
    if (user.avatar_url.startsWith('/uploads/')) { // Avatar yang diupload (disajikan backend)
      avatarSrc = `${BASE_URL}${user.avatar_url}`;
    } else if (user.avatar_url.startsWith('/img/')) { // Default avatar lokal dari DB
      avatarSrc = user.avatar_url;
    }
    // Anda bisa menambahkan kondisi lain jika avatar_url adalah URL eksternal penuh
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex flex-1 mx-4 max-w-xl">
            <div className="relative w-full">
              ...
              <input
                type="text"
                placeholder="Cari pin..." // Ganti placeholder jika perlu
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            ...
            <button className="p-2 rounded-full hover:bg-gray-100 hidden sm:block" title="Notifikasi">
              <FiBell className="text-gray-700 text-xl" />
            </button>
            <button className="p-2 rounded-full hover:bg-gray-100 hidden sm:block" title="Pesan">
              ...
              <>
                ...
                <div className="relative">
                  <button
                    ...
                    className="flex items-center p-1 rounded-full hover:bg-gray-100" // Tambah padding agar mudah diklik
                    aria-expanded={isProfileOpen}
                    aria-haspopup="true"
                    aria-controls="profile-menu"
                  >
                    {/* GANTI FiUser DENGAN Image AVATAR */}
                    <div className="relative w-8 h-8 rounded-full overflow-hidden bg-gray-200">
                      <Image
                        ...
                        layout="fill"
                        objectFit="cover"
                        key={avatarSrc} // Untuk re-render jika avatarSrc berubah
                      />
                    </div>
